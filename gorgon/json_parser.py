...
class GorgonJsonParser(object):
  ...
  def ParseItems(self):
    ...
    for item_id, item_data in data.iteritems():
      ...
      self.items[item_id] = model.Item(name=name, description=desc, id=item_id, required_appearance=required_appearance, skill_reqs=skill_reqs, value=value, icon=icon_id, effects=effects, slot=slot, keywords=keywords, version=self.version, stack_size=stack_size)
  ...
  def ParseTSysClientInfo(self):
    ...
    for power_id, power_data in data.iteritems():
      ...
      self.powers[power_id] = model.Power(prefix=prefix, suffix=suffix, tiers=tiers)
      for ability_name, skill_bonus, chance, flat_bonus, flat_chance, source in self.ExtractDamageMod(self.powers[power_id]):
        logging.info("Found mod %s(%s, %s, %s, %s) in %s", ability_name, skill_bonus, chance, flat_bonus, flat_chance, tiers[max(tiers.keys())][0])
  ...
  def FindBonusesInMod(self, mod):
    ...
    try:
      ...
      chance, skill_bonus = re.search("([0-9]+)% chance to deal \+([0-9]+)%(?: health(?: and(?: \+[0-9]+%)? armor)?)? damage", mod).groups()
      skill_bonus = int(skill_bonus) / 100.0
    ...
    try:
      ...
      flat_chance, flat_bonus = re.search("([0-9]+)% chance to deal \+([0-9]+)(?:(?: direct)? health)? damage", mod).groups()
  ...
  def ParseAttributes(self):
    ...
    for attribute, attr_data in data.iteritems():
      ...
      self.attributes[attribute] = model.Attribute(id=id_, name=attribute, label=label, is_hidden=is_hidden)
  ...
  def ParseSkills(self):
    ...
    for skill, skill_data in data.iteritems():
      ...
      self.skills[skill] = model.Skill(id=id_, name=name, description=desc, combat=combat, maxlevel=maxlevel, rewards=rewards, advtable=advtable, xptable=xptable, parents=parents)
  ...
  def ParseAbilities(self):
    ...
    for ability, ability_data in data.iteritems():
      ...
      self.abilities[id_] = model.Ability(id=id_, name=name, description=desc, damage=damage, damage_type=damage_type, internal_name=internal_name, range=range_, power_cost=power_cost, cooldown=cooldown, level=level, skill=skill)
  ...
  def ParseRecipes(self):
    ...
    for recipe, recipe_data in data.iteritems():
      ...
      recipe_obj = model.Recipe(id=id_, name=name, desc=description, iconid=iconid, internal_name=internal_name, skill=skill, skill_level_req=skill_level_req, reward_skill=reward_skill, reward_skill_xp_1st=reward_skill_xp_1st, reward_skill_xp=reward_skill_xp, ingredients=ingredients, results=results)
  ...
  def __sub__(self, other):
    new_items = dict([(id_, self.items[id_]) for id_ in self.items if id_ not in other.items])
    new_powers = dict([(id_, self.powers[id_]) for id_ in self.powers if id_ not in other.powers])
    new_abilities = dict([(id_, self.abilities[id_]) for id_ in self.abilities if id_ not in other.abilities])
